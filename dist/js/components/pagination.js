define(["jquery", "ractive", "text!../../templates/components/components.rac", 'ui', 'ractive-slide', 'ractive-tooltip', "jquery-pagination"],
    function ($, Ractive, template, UI, Slide, Tooltip) {
    ...
    UI.Pagination = UI.extend(UI, function (options) {
        ...
        var load = function () {
            ...
            if (me.localItems.length === 0) {
                ...
                $.ajax({
                    ...
                    // 中钢信后台，调整左侧side_nav高度
                    var sideNav = $('.side_nav'),
                        featureNav = $('.feature_nav'),
                        mainContent = $('.main_content');

                    if (typeof sideNav === 'object' &&
                        typeof featureNav === 'object' &&
                        typeof mainContent === 'object') {

                        var minHeight = parseInt(sideNav.css('min-height')),
                            currHeight = featureNav.height() + mainContent.height();

                        if (currHeight > minHeight) {
                            sideNav.height(currHeight);
                        } else {
                            sideNav.height(minHeight);
                        }
                    }
        ...
        this.setQueryParams = function (queryParams) {
            ractive.set("queryParams", queryParams);
            load();
        };
        ...
        this.get = function () {
            ractive.get.apply(ractive, arguments);
        };
        this.set = function () {
