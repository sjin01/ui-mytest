...
(function (root, factory)
{
    ...
    if (typeof exports === 'object') {
        ...
        define(['moment'], factory/*function (req)
        {
            console.dir(req);
            // Load moment.js as an optional dependency
            var id = 'moment';
...
}(this, function (moment)
{
    ...
    compareDatesForTime = function (a, b) {
        var _a = new Date(a.getTime());
        var _b = new Date(b.getTime());
        setToStartOfDay(_a);
        setToStartOfDay(_b);
    ...
    defaults = {
        ...
        // set in 'config' based on if showTime is true
        format: null,
    ...
    renderTimeSelect = function(numStart, numEnd, selectedVal, selectedClass, displayFunc)
    {
        var select = '<select class="pika-select ' + selectedClass + '">';
        for (var i = numStart; i < numEnd; i++) {
            select += '<option value="' + i + '" ' +
                (i == selectedVal ? 'selected' : '') + '>' +
                    displayFunc(i) + '</option>';
        }
        select += '</select>';
        return select;
    },

    renderTimeTable = function(opts, d)
    {
        var h = d ? d.getHours() : 0, m = d ? d.getMinutes() : 0, s = d ? d.getSeconds() : 0,
            hFrom = 0, mFrom = 0, sFrom = 0,
            hTo = 24, mTo = 60, sTo = 60,
            minDate = opts.minDate, maxDate = opts.maxDate;

        if (minDate && minDate instanceof Date) {
            hFrom = minDate.getHours();
            mFrom = minDate.getMinutes();
            sFrom = minDate.getSeconds();

            if (!!d) {
                if (compareDatesForTime(d, minDate) === 1) {
                    hFrom = 0;
                    mFrom = 0;
                    sFrom = 0;
                } else {
        ...
        if (maxDate && maxDate instanceof Date) {
            hTo = maxDate.getHours() + 1;
            mTo = maxDate.getMinutes() + 1;
            sTo = maxDate.getSeconds() + 1;

            if (!!d) {
                if (compareDatesForTime(d, maxDate) === -1) {
                    hTo = 24;
                    mTo = 60;
                    sTo = 60;
                } else {
        ...
        var table = renderTimeSelect(hFrom, hTo, h, 'pika-select-hour form-control', function(i) {
            if (opts.use24hour) {
                return (i < 10) ? '0' + i : i;
            } else {
                var result = (i % 12) + (i < 12 ? ' AM' : ' PM');
                if (result == '0 AM') {
        ...
        renderTimeSelect(mFrom, mTo, m, 'pika-select-minute form-control', function(i) {
            return (i < 10) ? '0' + i : i;
        });

        if (opts.showSeconds) {
            table += renderTimeSelect(sFrom, sTo, s, 'pika-select-second form-control', function(i) {
                return (i < 10) ? '0' + i : i;
            });
        }
        return table;
    ...
    Pikaday = function(options)
    {
        ...
        self._onMouseDown = function(e)
        {
            ...
            if (!hasClass(target.parentNode, 'is-disabled')) {
                if (hasClass(target, 'pika-button') && !hasClass(target, 'is-empty')) {
                    var newDate = new Date(target.getAttribute('data-pika-year'),
        ...
        self._onClick = function(e)
        {
            ...
            // why my IE8 has addEventListeners ???
            // defined in ractive-legacy.js
    ...
    Pikaday.prototype = {
        ...
        toString: function(format)
        {
            return !isDate(this._d) ? '' :
                hasMoment ? moment(this._d).format(format || this._o.format) :
                this._o.onlyShowTime ? this._d.toString() :
                this._o.showTime ?  this._d.toString() : this._d.toDateString();
        },
        ...
        {
            //setToStartOfDay(value); // delete for timepicker
        ...
        draw: function(force)
        {
            ...
            for (var c = 0; c < opts.numberOfMonths; c++) {
                ...
                if (!opts.onlyShowTime) {
                    html += renderTitle(this, c, this.calendars[c].year, this.calendars[c].month, this.calendars[0].year) +
                        this.render(this.calendars[c].year, this.calendars[c].month);
                }

                if (opts.showTime) {
                    html += '<div class="pika-time-container">' +
                        renderTimeTable(opts, this._d) +
                        '</div>';
                }
